  ...
  const handleSubmit = async () => {
    ...
    try {
      if (editingId) {
        ...
        setMembers((prev) => prev.map(m => m._id === editingId ? updated.data : m));
      } else {
        // Create new member
        const res = await fetch('/api/members', {
          method: 'POST',
                    ...
                    {members.map(m => (
                        <tr key={m._id} className="hover:bg-slate-50 dark:hover:bg-slate-700/30 transition-colors">
                            ...
                            <td className="px-6 py-4 text-center">
                                ...
                                <button onClick={() => handleDelete(m._id || '')} className="p-1 hover:text-red-500"><span className="material-icons-outlined text-xl">delete</span></button>
                            </td>
                        </tr>
                    ))}
                </tbody>
