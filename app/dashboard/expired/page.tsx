...
export default function ExpiredPage() {
  ...
  const handleSave = () => {
    ...
    const updatedList = assets.map((a) => (a.id === (updatedAsset as any).id ? updatedAsset : a));
    ...
    <div className="flex min-h-screen bg-[var(--background)] font-sans">
        ...
        <div className="bg-[var(--card-background)] rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 overflow-hidden">
            ...
            <div className="overflow-x-auto">
                <table className="w-full text-left border-collapse">
                    ...
                    <tbody className="divide-y divide-slate-100 dark:divide-slate-700/50 text-[var(--text)]">
                        {expiredAssets.length === 0 ? (
                            ...
                            expiredAssets.map((asset) => (
                                <tr key={asset._id || asset.id} className="hover:bg-slate-50/50 dark:hover:bg-slate-800/30 transition-colors text-[var(--text)] dark:text-[var(--text)]">
                                    <td className="px-4 py-4 text-sm font-medium">{asset.scCode}</td>
                                    <td className="px-4 py-4 text-sm">{asset.projectName}</td>
                                    <td className="px-4 py-4 text-center">
                                        <span className="px-2 py-1 text-[15px] font-bold rounded-full bg-red-100 text-red- dark:bg-red-900/30 dark:text-red-400">หมดประกัน</span>
                                    </td>
                                    <td className="px-4 py-4 text-center">
                                    ...
                                    <td className="px-4 py-4 text-center">
                                        <div className="flex justify-center gap-2">
                                            ...
                                            <button onClick={() => handleDelete(asset.id || 0)} className="p-1.5 text-slate-400 hover:text-red-500 transition-colors"><span className="material-icons-outlined text-lg">delete</span></button>
                                        </div>
                                    </td>
                                </tr>
                            ))
      ...
      {isModalOpen && (
          <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 z-50">
             <div className="bg-[var(--card-background)] rounded-2xl shadow-2xl w-full max-w-md p-6">
                <h2 className="text-xl font-semibold mb-6 text-[var(--text)]">แก้ไขข้อมูล (หมดประกัน)</h2>
                ...
                <div className="space-y-4">
                    ...
                    <input type="date" className="w-full p-2 border rounded dark:bg-slate-900 dark:border-slate-700 dark:text-white" value={formData.startDate as string} onChange={e => setFormData({...formData, startDate: e.target.value})} />
                    <textarea className="w-full p-2 border rounded dark:bg-slate-900 dark:border-slate-700 dark:text-white" value={formData.remark as string} onChange={e => setFormData({...formData, remark: e.target.value})} placeholder="Remark" />
                </div>
