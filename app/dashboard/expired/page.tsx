...
export default function ExpiredPage() {
  ...
  const handleSave = async () => {
    ...
    try {
      if (formData._id) {
        const res = await fetch(`/api/assets/${formData._id}`, {
          method: 'PUT',
        ...
        setAssets((prev) => prev.map((a) => (a._id === formData._id ? updated.data : a)));
      }
                                ...
                                <tr key={asset._id || asset.id} className="hover:bg-slate-50/50 dark:hover:bg-slate-800/30 transition-colors text-slate-700 dark:text-slate-200">
                                    ...
                                    <td className="px-4 py-4 text-center">
                                        <div className="flex justify-center gap-2">
                                            ...
                                            <button onClick={() => handleDelete(asset._id || '')} className="p-1.5 text-slate-400 hover:text-red-500 transition-colors"><span className="material-icons-outlined text-lg">delete</span></button>
                                        </div>
                                    </td>
                                </tr>
                            ))
